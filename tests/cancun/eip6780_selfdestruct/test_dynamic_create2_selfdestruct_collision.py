...
@pytest.fixture
def selfdestruct_contract_code(
    yul: YulCompiler,
...
def selfdestruct_contract_initcode(
    selfdestruct_contract_code: Bytecode, selfdestruct_contract_initcode_prefix: Bytecode
) -> Initcode:
    """Initcode of the selfdestruct contract."""
    return Initcode(
        deploy_code=selfdestruct_contract_code,
...
@pytest.fixture
def deployer_contract_code(create2_salt: int) -> Bytecode:
...
@pytest.fixture
def deployer_contract_address(pre: Alloc, deployer_contract_code: Bytecode) -> Address:
    """Address of the deployer contract."""
    return pre.deploy_contract(deployer_contract_code)


@pytest.fixture
def create2_storage_contract_code() -> Bytecode:
...
def create2_storage_contract_address(
    pre: Alloc, create2_storage_contract_code: Bytecode, create2_storage_contract_init_balance: int
) -> Address:
    """Address of the create2 storage contract."""
    return pre.deploy_contract(
        create2_storage_contract_code, balance=create2_storage_contract_init_balance
    )
...
def test_dynamic_create2_selfdestruct_collision(
    ...
    selfdestruct_contract_code: Bytecode,
    selfdestruct_contract_init_balance: int,
    selfdestruct_contract_initcode: Initcode,
    selfdestruct_recipient_address: Address,
    create2_storage_contract_address: Address,
    ...
    post[selfdestruct_contract_address] = (
        Account(
            balance=0,
            nonce=1,
            code=selfdestruct_contract_code,
...
def test_dynamic_create2_selfdestruct_collision_two_different_transactions(
    ...
    post[selfdestruct_contract_address] = (
        ...
        if create2_dest_already_in_state and fork >= Cancun
            ...
            Account.NONEXISTENT
            if call_create2_contract_at_the_end
            else Account(balance=second_create2_value, nonce=1, code=selfdestruct_contract_code)
        )
    )

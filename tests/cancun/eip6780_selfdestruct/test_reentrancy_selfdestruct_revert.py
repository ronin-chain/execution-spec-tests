...
@pytest.fixture
def selfdestruct_contract_code(selfdestruct_recipient_address: Address) -> Bytecode:
...
def selfdestruct_contract_address(
    pre: Alloc, selfdestruct_contract_code: Bytecode, selfdestruct_contract_init_balance: int
) -> Address:
    """Address of the selfdestruct contract."""
    print("selfdestruct_contract_code", selfdestruct_contract_code)
    return pre.deploy_contract(
        code=selfdestruct_contract_code, balance=selfdestruct_contract_init_balance
    )
...
def revert_contract_address(
    pre: Alloc,
    revert_contract_code: Bytecode,
    revert_contract_init_balance: int,
) -> Address:
    """Address of the revert contract."""
    return pre.deploy_contract(revert_contract_code, balance=revert_contract_init_balance)
...
@pytest.fixture
def executor_contract_code(
    first_suicide: Op,
    revert_contract_address: Address,
    selfdestruct_contract_address: Address,
) -> Bytecode:
    """Contract code that performs a selfdestruct call then reverts."""
    return (
...
def executor_contract_address(
    pre: Alloc,
    executor_contract_code: Bytecode,
...
def test_reentrancy_selfdestruct_revert(
    ...
    if first_suicide in [Op.CALL]:
        ...
        if fork >= Cancun:
            ...
            post[selfdestruct_contract_address] = Account(
                balance=0, code=selfdestruct_contract_code, storage={}
            )
        else:
